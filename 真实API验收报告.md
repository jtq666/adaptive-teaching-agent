...
本次自动化回归为 183 项，另有结构化复核用例覆盖单 Agent 四角色编排、内部任务语言泄露、变量重复赋值与多数值情境、概念焦点到操作阶段跳跃、结构化输出失败分类、数字审计字段归一化、真实页面焦点与问题一致性、“多种区间表示法同时出现”“互斥比较分支切换”、重复教师问题拦截、重复教学目标拦截、当前微步骤知识点边界、物理刹车方向守卫、导数极限困惑桥接、下一关注点多候选消歧、相关正向证据解锁、自然语言标点清理、快速示例与视图控件取消选择恢复、学生视图信息分层和修复失败后的通用回退。
...
真实快速实时验收结果：单轮 3 次 LLM 调用，耗时约 4.9 秒，状态保持 `active`；AI 推荐演示回答真实生成 3 条且无重复。前端 AppTest 23 项、浏览器验收和完整 183 项回归均通过。
...
本轮同时修复了开始教学后切换学生视图/教师视图时的 Streamlit 表单状态丢失问题，并补齐两个预设的前置知识字段，避免默认现场会话无故退回通用诊断。

设计系统整改后又完成一轮真实浏览器教学：首轮学生视图展示独立“本轮学习卡”，困惑回答后保持原情境与回答目标；答辩视图将“内容 Skill × 教学策略组合”和“本轮主执行角色”分开表达。最新验收截图保存在 `.e2e-runtime/screenshots/revisit-guard-20260816/`，包含 1920×1080、1366×768 和 390×844 视图，以及 Skill 切换、困惑回答和回放/评估入口证据。
